...
function Input({ className, type, ...props }: React.ComponentProps<'input'>) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        'placeholder:text-white/30 border-white/10 flex h-9 w-full min-w-0 rounded-md border bg-transparent px-3 py-1 text-base shadow-xs transition-[color,border-color,box-shadow] outline-none disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm',
        'focus-visible:border-white/25 focus-visible:ring-white/20 focus-visible:ring-[3px]',
        className
      )}
      {...props}
    />
...
export interface SignInCard2Props {
  /** Called when the user submits email + password. Return an error message or null. */
  onEmailSignIn?: (email: string, password: string) => Promise<string | null> | void;
  /** Called when the user clicks "Sign in with Google". */
  onGoogleSignIn?: () => void | Promise<void>;
  /** External error message shown above the form. */
  error?: string | null;
  /** App name shown under the heading. Defaults to "CodeSync". */
  appName?: string;
}
...
export function SignInCard2({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (onEmailSignIn) {
        ...
        // Demo fallback when no handler is wired up.
  ...
  return (
    <div className="min-h-screen w-screen bg-black relative overflow-hidden flex items-center justify-center">
      {/* Background gradient effect - blue OnlyPipe style */}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {error && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="p-2.5 rounded-lg bg-red-500/10 border border-red-500/30 text-red-300 text-xs text-center"
                  >
                    {error}
                  </motion.div>
                )}
                ...
                >
                  ...
                  <Link href="/auth/signup" className="relative inline-block group/signup">
